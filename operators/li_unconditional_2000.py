"""
Unconditional Li coefficients via direct Cauchy DFT — N=2000, properly calibrated.

CALIBRATION:
  For N=2000: k*=141.5, K_COMPUTE=390 (tail<10^{-15}), dps=210 (margin ~34 dps at k=390).
  Formula: dps_needed = K * log10(14.136/5) + 35 = 390 * 0.4513 + 35 = 210.

RUNTIME ESTIMATE: ~5-8 minutes.
"""
...
for k in [50, 100, 200, 300, 390]:
    decay_expect = k * 14.136**(-k)
    pv = float(p[k])
    if abs(pv) > 0 and decay_expect > 0:
...
print(f"\nVerification:")
known = {1: 0.023095708966, 10: 2.279339363193, 500: 991.9000929922, 1000: 2326.0531535}
for n, kn in known.items():
    if n <= N_LAMBDA:
        computed = lambda_vals[n-1]
        rel = abs(computed - kn)/abs(kn)
        print(f"  n={n}: {computed:.12f}  (known {kn:.6f})  rel_err={rel:.2e}")
...
results = {
    ...
    'spot_checks': {str(n): lambda_vals[n-1] for n in [1,5,10,50,100,500,1000,1500,2000] if n <= N_LAMBDA},
    'timing': {'eval_s': eval_time, 'dft_s': dft_time, 'lambda_s': lambda_time},
}
outfile = os.path.join(RESULTS_DIR, 'li_unconditional_2000.json')
